    ...
    func retrieveUrls(forAnnotation annotation: DroppablePin, handler: @escaping (_ status: Bool) -> ()) {
        Alamofire.request(flickerUrl(forApiKey: apiKey, withAnnotation: annotation, andNumberOfPhotos: 40)).responseJSON { (response) in
            ...
            for photo in photosDictArray {
                ...
                let owner = "Owner: \(photo["owner"]!)"
                self.ownerArray.append(owner)
                print(title)
            }
            handler(true)
        }
...
extension MapVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        popVC.initData(forImage: imageArray[indexPath.row], title: titleArray[indexPath.row], owner: ownerArray[indexPath.row])
